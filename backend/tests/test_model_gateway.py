...
class TestModelGateway:
    ...
    async def test_embed_single(self, gateway: ModelGateway):
        ...
        with patch.object(ModelGateway, "_post", new_callable=AsyncMock, return_value=mock_response):
    ...
    async def test_health_check(self, gateway: ModelGateway):
        with patch.object(ModelGateway, "_get", new_callable=AsyncMock, return_value={"status": "ok"}):
